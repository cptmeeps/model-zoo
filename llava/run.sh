curl -o ViT-L-14-336px.pt https://cptmeep-public.s3.us-west-2.amazonaws.com/model_zoo/clip/ViT-L-14-336px.pt
curl -o bpe_simple_vocab_16e6.txt.gz https://cptmeep-public.s3.us-west-2.amazonaws.com/model_zoo/clip/bpe_simple_vocab_16e6.txt.gz
curl -o tokenizer.model https://cptmeep-public.s3.us-west-2.amazonaws.com/model_zoo/llama-2-7b/tokenizer.model
curl -o alpaca.json https://cptmeep-public.s3.us-west-2.amazonaws.com/datasets/alpaca_data_cleaned.json
curl -o consolidated.00.pth https://cptmeep-public.s3.us-west-2.amazonaws.com/model_zoo/llama-2-7b/consolidated.00.pth


curl -o llava/data/chat.json https://cptmeep-public.s3.us-west-2.amazonaws.com/model_zoo/llava/chat.json
curl -o metadata.json https://cptmeep-public.s3.us-west-2.amazonaws.com/model_zoo/llava/metadata.json
curl -o images.zip https://cptmeep-public.s3.us-west-2.amazonaws.com/model_zoo/llava/images.zip
