import os, math, time, random, json, requests
...
def print_memory_usage(stage):
    print(
      f"{stage}:",
      f"alloc: {torch.cuda.memory_allocated('cuda')/1024**3:.1f}",
      f"cached: {torch.cuda.memory_reserved('cuda')/1024**3:.1f}",
    )

def reg_mem_hooks(module, module_name_prefix='', sub=False):
  def forward_hook(module, input, output):
    print(
      f"{module_name_prefix} - forward:",
      f"alloc: {torch.cuda.memory_allocated('cuda')/1024**3:.1f}",
      f"cached: {torch.cuda.memory_reserved('cuda')/1024**3:.1f}",
    )
  def backward_hook(module, grad_input, grad_output):
    print(
      f"{module_name_prefix} - backward:",
      f"alloc: {torch.cuda.memory_allocated('cuda')/1024**3:.1f}",
      f"cached: {torch.cuda.memory_reserved('cuda')/1024**3:.1f}",
    )
  ...
  if sub:
    for name, child in module.named_children():
      ...
      reg_mem_hooks(child, new_prefix, sub)

def reg_shape_hooks(module, module_name_prefix='', sub=False):
  def forward_hook(module, input, output):
    ...
    print(f"{module_name_prefix} - forward:", f"in: {input_shapes}", f"out: {output_shapes}")
  
  def backward_hook(module, grad_input, grad_output):
    grad_input_shapes = [ginp.shape for ginp in grad_input if isinstance(ginp, torch.Tensor)]
    grad_output_shapes = [gout.shape for gout in grad_output if isinstance(gout, torch.Tensor)]
    print(f"{module_name_prefix} - backward:", f"in: {grad_input_shapes}", f"output: {grad_output_shapes}")
  ...
  if sub:
    for name, child in module.named_children():
      new_prefix = f"{module_name_prefix}.{name}" if module_name_prefix else name
      reg_shape_hooks(child, new_prefix, sub)
...
def generate(model, tokenizer, model_args, prompt_toks, image=None, max_gen=32):
...
def test_text():
  ...
  model_args = ModelArgs(max_batch_size=4, max_seq_len=32)
  model = build(model_args)
  # register_hooks(model)
  text_toks = [tokenizer.encode(x, bos=True, eos=False) for x in prompts['txt']]
  text_out_toks = generate(model, tokenizer, model_args, text_toks, image=None)
  [print(tokenizer.decode(t)) for t in text_out_toks]
